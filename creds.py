class Creds():
    # ENTER Your bot Token Here
    TG_TOKEN = "1224001877:AAGhcHpplRV72fvKHMsLnob_p78NW8fhnhg"
    
    
    
    #  Make Sure You Are Providing both value if you need Teamdrive upload
    # Because of pydrive And pydrive v2 Api
    
    #Folder Id Of Teamdrive
    TEAMDRIVE_FOLDER_ID = "1cGYxEhCtSCe-jOSxMtXjoHmZVFUdX5op?usp=sharing"
    
    # Id of Team drive 
    TEAMDRIVE_ID = ""
    
    
    
    #Example 
    #TG_TOKEN = "1HthfqXtr9csJY9Ke47OqK_QLMXczu6aX"
    #TEAMDRIVE_FOLDER_ID = "13v4MaZnBz-iEHlZ0gFXk7rh"
    #TEAMDRIVE_ID = "0APh6R4WVvguEUk9PV"
